---
title: Hugo Phish — Übersicht
description: Phishing-Simulation für deutsche Unternehmen — Programm läuft von allein, Betriebsrat sagt ja, Prüfer bekommt sein PDF.
---

**Hugo Phish** trainiert Ihre Belegschaft mit realistischen Phishing-Testmails — automatisch geplant,
datenschutzkonform und mit auditfestem Nachweis. Anders als klassische Awareness-Schulungen messen wir
nicht, wer versagt, sondern wer **schützt**: die Melderate ist die Leitkennzahl.

![Cockpit der Phishing-Simulation: die vier Kennzahlen Melderate (30 %), Klickrate (19 %), Resilienz-Index (74 von 100) und Öffnungsrate (79 %), darunter der Resilienz-Verlauf über die bisherigen Wellen, der Zustellbarkeits-Status für Microsoft 365 und die Liste der vier abgeschlossenen Kampagnen](/screenshots/hugo-phish/cockpit.jpg)

## Die Bausteine

| Bereich | Wofür |
|---|---|
| [Schnellstart](/hugo-phish/schnellstart/) | In fünf Schritten zur ersten Kampagne |
| [Zustellbarkeit](/hugo-phish/zustellbarkeit/) | Allowlisting, damit die Mails ankommen (Pflicht) |
| [Jahresprogramm](/hugo-phish/programm/) | Der Autopilot: Wellen, Rhythmus, Sperrzeiten |
| [Betriebsrat & Datenschutz](/hugo-phish/betriebsrat/) | Anonymität, Musterbetriebsvereinbarung, Rechtsgrundlagen |
| [Berichte & Nachweis](/hugo-phish/berichte/) | Report-PDF, Audit-Paket für NIS2/ISO/Versicherung |
| [Häufige Fragen](/hugo-phish/faq/) | Kurz beantwortet |

## Was Hugo Phish besonders macht

- **Melderate statt Klick-Pranger.** Wer eine verdächtige Mail meldet, wird sichtbar belohnt.
- **Betriebsrats-Modus, technisch erzwungen.** Auswertungen nur in Gruppen ab 5 Personen — kein
  Administrator sieht Einzelnamen. Das ist in der Datenbank gesperrt, nicht nur zugesagt.
- **Sofort-Schulung im Moment des Fehlers.** Ein Klick löst binnen Sekunden eine kurze Auffrischung aus.
- **Nachweis auf Knopfdruck.** Audit-Paket mit Zuordnung zu DSGVO Art. 32, ISO 27001 A.6.3, NIS2 und
  § 391 SGB V.
